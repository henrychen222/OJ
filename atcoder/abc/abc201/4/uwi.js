...
// Accepted --- 393ms https://atcoder.jp/contests/abc201/submissions/23463259
const solve = (n, m, g) => {
    // pr(n, m, g);
    let dp = initialize2DArrayNew(n, m);
    for (let i = n - 1; ~i; i--) { // bottom-up dp
        for (let j = m - 1; ~j; j--) {
            if (i == n - 1 && j == m - 1) continue;
            let right = j + 1 < m ? -dp[i][j + 1] + (g[i][j + 1] == '+' ? 1 : -1) : Number.MIN_SAFE_INTEGER / 2;
            let down = i + 1 < n ? -dp[i + 1][j] + (g[i + 1][j] == '+' ? 1 : -1) : Number.MIN_SAFE_INTEGER / 2;
            dp[i][j] = mx(right, down);
        }
    }
    // pr(dp);
    pr(dp[0][0] > 0 ? 'Takahashi' : dp[0][0] < 0 ? 'Aoki' : 'Draw');
};
