 ...
 */
// reference: https://atcoder.jp/contests/abc214/submissions/25019816
// Accepted --- https://atcoder.jp/contests/abc214/submissions/25059337
const solve = (s, t) => {
    let res = 0;
    for (let a = 0; a <= s; a++) {
        for (let b = 0; b <= s; b++) {
...
const solve1 = (s, t) => {
    let se = new Set();
    for (let a = 0; a <= s && a <= t; a++) {
        for (let b = 0; b <= s && b <= t; b++) {
            for (let c = 0; a + b + c <= s && a * b * c <= t; c++) {
                // pr(a, b, c);
                let tmp = a + ' ' + b + ' ' + c;
                se.add(tmp);
            }
