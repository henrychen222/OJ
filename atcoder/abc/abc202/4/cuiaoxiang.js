...
const initialize2DArrayNew = (m, n) => { let data = []; for (let i = 0; i < m; i++) { let tmp = new Array(n).fill(0n); data.push(tmp); } return data; };
...
// Accepted --- https://atcoder.jp/contests/abc202/submissions/22842956 69ms
// https://atcoder.jp/contests/abc202/submissions/22807460
const N = 61;
const solve = (A, B, k) => {
    // pr(A, B, k)
    let f = initialize2DArrayNew(N, N);
    for (let i = 0; i < N; i++) {
        f[i][0] = f[i][i] = 1n; // cnt
        for (let j = 1; j < i; j++) {
    ...
    let res = '';
    while (A || B) {
        if (A && (f[A + B - 1][A - 1] || 0n) >= k) {
            res += 'a';
            A--;
        } else {
            res += 'b';
            // pr(k, A + B - 1, A - 1, f[A + B - 1][A - 1])
            k -= (f[A + B - 1][A - 1] || 0n); // c++ out of bound will return 0;
            B--;
        }
    }
    pr(res);
...
const main = () => {
    ...
    rl.on('close', () => {
        solve(Number(input[0][0]), Number(input[0][1]), ll(input[0][2])); // WA big_04 reason, k is BigInt, not number
    });
};
