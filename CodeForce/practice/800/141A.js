 ...
 */
// Accepted --- 156ms https://codeforces.com/problemset/submission/141/121674172
const solve = (a, b, c) => {
    // pr(a, b, c)
    let cm = counter(c);
    for (const [k, occ] of a) {
        if (!cm.has(k)) return pr('NO');
        let cur = cm.get(k);
        if (cur == 1) {
            cm.delete(k);
        } else {
