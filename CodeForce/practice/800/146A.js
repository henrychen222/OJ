 ...
 */
// Accepted --- 154ms https://codeforces.com/problemset/submission/146/121671921
const solve = (n, s) => {
    // pr(n, s)
    for (const c of s) {
        if (c != '4' && c != '7') return pr('NO');
    }
    let half = n / 2;
    let lsum = rsum = 0;
    for (let i = 0; i < half; i++) lsum += s[i] - '0';
    for (let i = half; i < n; i++) rsum += s[i] - '0';
    // pr(lsum, rsum);
    if (lsum != rsum) return pr('NO');
    pr('YES');
};
