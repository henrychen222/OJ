 ...
 */
// WA, Idle TLE, interactive not work
// see Java https://codeforces.com/contest/1546/submission/122142090
const solve = (n, m, a, b) => {
    // pr(n, m, a, b)
    for (const s1 of a) {
        let flag = false;
...
const main = () => {
    ...
    // don't know how to do interactive (flush)
    // https://stackoverflow.com/questions/12510835/stdout-flush-for-nodejs
    // https://github.com/nodejs/node-v0.x-archive/issues/1669
    rl.on('pause', () => {
        let t = Number(next());
        while (t--) {
            let tmp = next().split(" ").map(Number);
            let [n, m] = tmp;
            let a = [];
            for (let i = 1; i <= n; i++) a.push(next());
            let b = [];
            for (let i = 1; i <= n - 1; i++) b.push(next());
            solve(n, m, a, b);
        }
