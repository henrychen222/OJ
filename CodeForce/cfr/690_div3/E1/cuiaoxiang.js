...
// Accepted --- https://codeforces.com/contest/1462/submission/128095025 1590ms
const combination2 = (m, n) => { let res = 1n; for (let i = 0; i < n; i++) res *= ll(m) - ll(i); for (let i = 0; i < n; i++) res /= ll(i + 1); return res; };
const solve = (n, a) => {
    let m = 3, k = 2;
    let freq = Array(n).fill(0);
    for (const x of a) freq[x - 1]++;
    let pre = Array(n + 1).fill(0);
    for (let i = 0; i < n; i++) pre[i + 1] = pre[i] + freq[i];
    let res = 0n;
    for (let i = 0; i < n; i++) {
        let r = mi(i + k, n - 1);
        let tot = pre[r + 1] - pre[i];
        let cur = combination2(tot, m);
        if (tot > freq[i]) cur -= combination2(tot - freq[i], m);
        res += cur;
    }
    pr(res.toString())
};

// Accepted --- https://codeforces.com/contest/1462/submission/128094536 1559ms
const solve1 = (n, a) => {
    // pr(n, a);
    let m = 3, k = 2;
    let freq = Array(n).fill(0);
    ...
    for (let i = 0; i < n; i++) {
        let r = mi(i + k, n - 1);
        let tot = pre[r + 1] - pre[i];
        let cur = combination(tot, m);
        // pr(cur);
        if (tot > freq[i]) cur -= combination(tot - freq[i], m);
        res += cur;
    }
    pr(res.toString())
};
