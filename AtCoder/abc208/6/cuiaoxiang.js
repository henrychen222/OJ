...
const bmod = ll(mod);
const N = 3000000;
const ifact = Array(N).fill(0n);
const inv = Array(N).fill(0n);
const fact = Array(N).fill(0n);

// TLE
const solve = (n, m, k) => { // n: long  m: int  k: int
    // pr(n, m, k);
    let a = Array(k + m + 2).fill(0n);
    // pr(a);
    for (let i = 0; i < a.length; i++) a[i] = powmod(ll(i), ll(k), bmod);
    for (let j = 0; j < m; j++) {
        let na = Array(a.length).fill(0n);
        na[0] = a[0];
        // pr(na)
        for (let i = 1; i < a.length; i++) {
            // pr(a[i], na[i - 1])
            na[i] = (na[i - 1] + a[i]) % bmod;
        }
        a = na;
    }
    // pr(a);
...
const large_range = (a, n) => { // n: long
    ...
    for (let i = 0; i < m; i++) {
        if ((n - ll(i)) % bmod == 0) {
            ret = 1n * ifact[i] * ifact[m - 1 - i] % bmod * a[i] % bmod;
            if ((m - 1 - i) % 2) res = bmod - res;
            break;
        }
        // pr(ifact[i], ifact[m - 1 - i], a[i])
        let cur = ifact[i] * ifact[m - 1 - i] % bmod * powmod(n - ll(i), bmod - 2n, bmod) % bmod * a[i] % bmod;
        if ((m - 1 - i) % 2) cur = bmod - cur;
        res += cur;
    }
    res %= bmod;
    for (let i = 0; i < m; ++i) {
        if ((n - ll(i)) % bmod == 0) continue;
        res = (n - ll(i)) % bmod * res % bmod;
    }
    return res;
};
...
const comb_init = () => {
    inv[1] = 1n;
    for (let i = 2; i < N; i++) {
        inv[i] = (bmod - bmod / ll(i)) * inv[bmod % ll(i)] % bmod;
    }
    fact[0] = ifact[0] = 1n;
    for (let i = 1; i < N; i++) {
        fact[i] = fact[i - 1] * ll(i) % bmod;
        ifact[i] = ifact[i - 1] * inv[i] % bmod;
    }
};
...
const main = () => {
    ...
    rl.on('close', () => {
        comb_init();
        solve(ll(input[0][0]), Number(input[0][1]), Number(input[0][2]));
    });
};
