...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 06/13/21 morning
// Accepted --- 1297ms  https://atcoder.jp/contests/abc205/submissions/23455414
const solve = (n, q, a, k) => {
    // pr(n, q, a, k)
    for (let i of k) {
        if (i <= a[0] - 1n) {
        ...
        let [low, high] = [0n, n - 1n];
        while (low != high) {
            let mid = (low + high) / 2n;
            let r = a[mid + 1n] - (mid + 2n);
            // pr("r", r, i > r);
            i > r ? low = mid + 1n : high = mid;
        }
        let cur = a[high] - (high + 1n);
        i -= cur;
        pr((a[high] + i).toString());
    }
}
...
const main = () => {
    ...
    rl.on('close', () => {
        ...
        let a = input.shift();
        let k = [];
        for (let i = 0; i < q; i++) k.push(input[i][0]);
        solve(n, q, a, k);
    });
};
