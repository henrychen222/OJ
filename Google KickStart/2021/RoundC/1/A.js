...
const solve = (n, k, s) => {
    ...
    let se = new Set();
    for (let i = 0; i < k; i++) {
        se.add(String.fromCharCode(97 + i));
    }
    // pr(se);
    ...
    for (let i = n - 1; ~i; i--) {
        for (const c of se) {
            let a = [...tmp];
            // if (c.charCodeAt() >= a[i].charCodeAt()) continue;
            a[i] = c;
            let ss = a.join("");
            pr(ss);
            if (exist.has(ss)) continue;
            if (isSmaller(ss, s) && ss != s) {
                if (isPalindrome(ss)) {
                    // pr(ss);
                    res++;
                }
...
const main = () => {
    ...
    rl.on('close', () => {
        ...
        for (let cas = 1, i = 1; cas <= t; cas++, i += 2) {
            let tmp = input[i].split(" ").map(Number);
            let show = solve(tmp[0], tmp[1], input[i + 1]);
            pr('Case #' + cas + ': ' + show);
        }
    });
};
